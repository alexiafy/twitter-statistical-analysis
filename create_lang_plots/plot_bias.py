...
db1 = connection1.fake_news                     # name of db (it should be changed to real_news or fake_news
    ...
    for tweet in tweets:
        ...
        if replies_count > 150:
            all_tweets1 += 1
                ...
                #contr_counter1 += 1
            print(tweet["language_scores"]["bias_score"])
            real_dict[tweet["id"]] = tweet["language_scores"]["bias_score"]
...
for collection in collections2:
    ...
    for tweet in tweets:
        ...
        if replies_count > 150:
            ...
            if(tweet["ratios"]["pn"]) > 0.5:
                contr_counter2 += 1
            fake_dict[tweet["id"]] = tweet["language_scores"]["bias_score"]
...
plt.title('Real news - Language score using bias lexicon (tweets with more than 150 replies)')
plt.xlabel('Tweet id')
plt.ylabel('Score')
plt.grid(linestyle=':')
plt.tight_layout()

plt.savefig("../plots/Language plots/Real news - Language score using bias lexicon.svg")
plt.show()
...
plt.title('Fake news - Language score using bias lexicon (tweets with more than 150 replies)')
plt.xlabel('Tweet id')
plt.ylabel('Score')
plt.grid(linestyle=':')
plt.tight_layout()

plt.savefig("../plots/Language plots/Fake news - Language score using bias lexicon.svg")
plt.show()
