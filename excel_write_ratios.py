...
workbook = xlsxwriter.Workbook('output/Real News - ratios.xlsx')
...
worksheet.write('H1', 'PN ratio', bold)
worksheet.write('I1', 'RPN ratio', bold)
worksheet.write('J1', 'PNPNT ratio', bold)
...
for collection in collections:
    ...
    for tweet in tweets:
        ...
        if replies_count > 0:
            ...
            worksheet.write_number(row, col + 7, tweet["ratios"]["pn"])
            worksheet.write_number(row, col + 8, tweet["ratios"]["rpn"])
            worksheet.write_number(row, col + 9, tweet["ratios"]["pnpnt"])
            row += 1
