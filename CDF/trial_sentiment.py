    ...
    for collection in collections:
        ...
        for tweet in tweets:
            ...
            if replies_count > 150:
                tweets_score.append(tweet["replies_sentiment"]["negative_replies"]/tweet["replies_count"])
    return tweets_score
...
plt.title('ECDF Plot of neg ratio')
plt.xlabel('Percent of tweets')
plt.ylabel('Neg ratio')
plt.yticks(np.arange(0, 1.1, 0.1))
plt.grid(linestyle=':')
legend = plt.legend(loc=6, bbox_to_anchor=(1, 0.5))


plt.savefig("../plots/CDF plots/Negative ratio.svg", bbox_extra_artists=(legend,), bbox_inches='tight')
plt.show()
