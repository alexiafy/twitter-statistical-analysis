    ...
    for tweet in tweets:
        ...
        if replies_count > 150:
            ...
            if(tweet["scores"]["sentiment_score"]) > 0.5:
                contr_counter1 += 1
            real_dict[tweet["id"]] = tweet["scores"]["sentiment_score"]
...
for collection in collections2:
    ...
    for tweet in tweets:
        ...
        if replies_count > 150:
            ...
            if(tweet["scores"]["sentiment_score"]) > 0.5:
                contr_counter2 += 1
            fake_dict[tweet["id"]] = tweet["scores"]["sentiment_score"]
...
plt.title('Real news - Sentiment score (tweets with more than 150 replies)')
plt.xlabel('Tweet id')
plt.ylabel('RPN ratio')
plt.grid(linestyle=':')
plt.tight_layout()

plt.savefig("../plots/Real news - Sentiment score.svg")
plt.show()
...
plt.title('Fake news - Sentiment score (tweets with more than 150 replies)')
plt.xlabel('Tweet id')
plt.ylabel('RPN ratio')
plt.grid(linestyle=':')
plt.tight_layout()

plt.savefig("../plots/Fake news - Sentiment score.svg")
plt.show()
