    ...
    for tweet in tweets:
        ...
        if replies_count > 150:
            ...
            if(tweet["ratios"]["pn"]) > 0.5:
                contr_counter1 += 1
            real_dict[tweet["id"]] = tweet["ratios"]["pn"]



print("Real - Ποσοστό των replies με ratio μεγαλύτερο από 0.5", contr_counter1/all_replies1*100)
...
for collection in collections2:
    ...
    for tweet in tweets:
        ...
        if replies_count > 150:
            ...
            if(tweet["ratios"]["pn"]) > 0.5:
                contr_counter2 += 1
            fake_dict[tweet["id"]] = tweet["ratios"]["pn"]
...
plt.title('Real news - PN ratio (tweets with more than 150 replies)')
plt.xlabel('Tweet id')
plt.ylabel('RPN ratio')
plt.grid(linestyle=':')
plt.tight_layout()

plt.savefig("plots/Real news - PN ratio.svg")
plt.show()
...
plt.title('Fake news - PN ratio (tweets with more than 150 replies)')
plt.xlabel('Tweet id')
plt.ylabel('RPN ratio')
plt.grid(linestyle=':')
plt.tight_layout()

plt.savefig("plots/Fake news - PN ratio.svg")
plt.show()
